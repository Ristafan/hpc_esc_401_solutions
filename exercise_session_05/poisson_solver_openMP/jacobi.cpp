...
void jacobi_step(params p, double** u_new, double** u_old, double** f) {
    ...
    #pragma omp parallel for
    for (int i = 0; i < p.nx; i++) {
        u_new[i][0] = boundary(i * dx, 0, p.rhs_function);
        u_new[i][p.ny - 1] = boundary(i * dx, 1, p.rhs_function);
    }

    #pragma omp parallel for
    for (int j = 0; j < p.ny; j++) {
        u_new[0][j] = boundary(0, j * dy, p.rhs_function);
        u_new[p.nx - 1][j] = boundary(1, j * dy, p.rhs_function);
    }
}
